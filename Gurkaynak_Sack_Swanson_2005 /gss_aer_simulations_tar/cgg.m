...
%end ;
%
%voltermstruct = sqrt(squeeze(varx(neq*(nlag-1)+3,neq*(nlag-1)+3,:))) ;
%
%for i=1:57 ;
%  voltermstruct1y(i,1) = mean(voltermstruct(i:i+4)) ;
%end ;
...
%  % Calculate term structure response to the shock:
%  for i = 1:40 ;
%    termstruct(i,1) = mean(x(nlag+1:nlag+i,3)) ;
%  end ;
%  
%  % Plot term structure response to the shock:
%  figure(2) ;
%  subplot(2,1,1); plot([0:NPERIODS-1]',x(nlag+1:nlag+NPERIODS,3),...
%				    [1:NPERIODS]',zeros(NPERIODS,1),'--k') ;
%    title('Forward Rate response out to 40 quarters') ;
%  subplot(2,1,2); plot([1:NPERIODS]',termstruct,...
%				    [1:NPERIODS]',zeros(NPERIODS,1),'--k') ;
%    title('Term Structure response out to 40 quarters') ;
