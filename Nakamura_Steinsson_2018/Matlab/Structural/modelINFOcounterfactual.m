function [FF,RR,eu] = modelINFOcounterfactual(yy,par,varargin)

% Routine that computes the matrices G1 and the error coefficients RR based
% on Gensys routine. The model is the Information Model of NS2015, for the
% counterfactual case in which the FED does not reveal information in 
% advance about the natural rate. It also outputs a vector eu to verify 
% existence and uniqueness of a solution.
%
% Takes as inputs:
% - yy: vector of parameters
% - policy: switch between optimal MP and MP without natural rate of int.
...
checkPar = @(x) isstruct(x);

addRequired(parsedInputs,'yy',checkParameters);
addRequired(parsedInputs,'par',checkPar);
addParameter(parsedInputs,'monetarypolicy',defaultMonetaryPolicy,checkMonetaryPolicy)

parse(parsedInputs,yy,par,varargin{:})

if strcmp(parsedInputs.Results.monetarypolicy,'optimal')
    policy = 1;
elseif strcmp(parsedInputs.Results.monetarypolicy,'alternative')
    policy = 0;
end
...
pi = zeros(dimg0,9);     % Expectational Errors
psi = zeros(dimg0,2);    % Disturbances
...
g0(2,15) = -kappa*zeta;
%g0(2,5)  = kappa*omega*zeta;   % Cost channel
...
if policy == 1
    g0(3,12) = -1;           % (r^n_t) if optimal MP
elseif policy == 0;
    g0(3,12) = 0;            % (r^n_t) if MP without natural rate
end
...
g0(14,14) = -(1+bb+bb^2*beta);
g0(14,29) = 1+beta*bb+bb^2 * beta;
g0(14,24) = -beta*bb;
g1(14,14) = -bb;
g0(14,12) = -sigma_c^(-1);
%g0(14,28) = -phi*phi_c;
...
% Expected natural output at t+2
g0(24,29) = 1;
g1(24,24) = 1;
pi(24,9) = 1;
